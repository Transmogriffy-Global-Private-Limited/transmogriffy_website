...
class ProductDetailsSchema(BaseModel):
    ...
    fast_charger: str = Field(..., description="Fast charger support ('yes' or 'no')")
    protection: str = Field(..., description="Protection type (e.g., 'Overload, Short Circuit')")
    communication: str = Field(..., description="Communication protocol (e.g., 'Modbus, CAN')")
    ...
    cooling: str = Field(..., description="Cooling method (e.g., 'Air cooled')")
    ingress_protection: str = Field(..., description="Ingress protection rating (e.g., 'IP65')")
    ...
    def validate_efficiency(cls, v):
        if not (0 <= v <= 100):
            raise ValueError("efficiency_in_percentage must be between 0 and 100")
        return v
    ...
    def validate_temperature(cls, v):
        if v < -100 or v > 200:
            raise ValueError("maximum_operating_temperature must be in a reasonable range (-100 to 200 °C)")
        return v
